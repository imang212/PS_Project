...
import yt_dlp
import paramiko
# Zde se předpokládá, že Picamera2 importujete pouze v případě, že se provádí na Raspberry Pi
# V rámci třídy OnboardRaspberryPiCameraProvider je ale pevný import.
# Pokud víte, že tento kód poběží jen na klasickém PC/Windows, můžete následující řádky s picamera2 smazat.
try:
    from picamera2 import Picamera2
except ImportError:
    # Ignorujeme, pokud Picamera2 není k dispozici (běžíme na PC)
    # Třída OnboardRaspberryPiCameraProvider nicméně selže, pokud se ji pokusíte použít.
    pass
...
class VideoStream:
    ...
    @property
    def is_file(self):
        # Tato část kódu vyžaduje, aby provider měl vlastnost .cap, kterou VideoStream nemá definovanou
        # Nicméně pro zachování původní funkčnosti to necháme, ale pozor na potenciální chybu, pokud by provider nepoužíval cv.VideoCapture.
        try:
             return isinstance(self._video_provider.get_name(), str) and self._video_provider.get_name() != ""
        except:
             return False
    ...
    def device_name(self):
        # Původní kód zde měl self.cap.getBackendName(), což by selhalo. Opravíme na providera.
        return self._video_provider.get_name()
    ...
    def update(self):
        ...
        # Inicializace bufferů při prvním platném snímku
    ...
    def release(self):
        # Předpokládáme, že provider má metodu release()
        if hasattr(self._video_provider, 'release'):
            self._video_provider.release()
...
class CameraVideoProvider(VideoProvider):
    ...
    def get_name(self) -> str:
        # Tady je potřeba použít vlastnost cap
        return self.cap.getBackendName() if self.cap.isOpened() else f"Camera (src={self.cap.get(cv.CAP_PROP_POS_FRAMES)})"
    ...
    def read(self) -> np.ndarray:
        ...
        if not ret:
            # Zavřeme, pokud se video soubor přehrál do konce
            if self.cap.get(cv.CAP_PROP_FRAME_COUNT) > 0 and self.cap.get(cv.CAP_PROP_POS_FRAMES) >= self.cap.get(cv.CAP_PROP_FRAME_COUNT):
                 self.cap.release()
            return None
        return frame
...
class FileVideoProvider(VideoProvider):
    ...
    def read(self) -> np.ndarray:
        ret, frame = self.cap.read()
        if not ret:
            if self.cap.get(cv.CAP_PROP_POS_FRAMES) >= self.cap.get(cv.CAP_PROP_FRAME_COUNT) - 1:
                self.cap.release() # Uvolníme zdroj po dočtení souboru
            return None
        return frame
...
class YouTubeVideoProvider(VideoProvider):
    ...
    def _get_stream_url(self, url: str) -> str:
        ...
        import os # Zde se import os musí opakovat, protože byl v původním, smazaném bloku
    ...
    def _download_video(self, save_path: str) -> str:
        ...
        import os # Import os je zde také potřeba
...
class OnboardRaspberryPiCameraProvider(VideoProvider):
    def __init__(self, resolution: tuple[int, int] = (640, 480), framerate: int = 30):
        # Tato třída je závislá na Picamera2
        # Pokud se Picamera2 nepodařilo importovat na začátku souboru (tj. běžíte na PC), zde nastane chyba
...
class RTPSStream(VideoStreamListener):
    ...
    def on_frame(self, frame: np.ndarray, formatted_frame: np.ndarray, stream: VideoStream) -> None:
        if self.writer is None:
            ...
            fps = 30 # Tuto hodnotu by bylo lepší brát z providera, ale ponecháme 30 pro jednoduchost.
